...
function StatCard({ icon: Icon, label, value, sub, color }) {
  return (
    <div className="bg-white rounded-xl border border-ink-100 p-5 flex items-start gap-4 animate-fade-up opacity-0-init">
      ...
      <div>
        <p className="text-ink-400 text-xs uppercase tracking-widest mb-0.5">
          {label}
        </p>
...
export default function Dashboard() {
  ...
  const styles = [
    ...new Set(designs.map((d) => d.garmentType).filter(Boolean)),
  ];
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-6 sm:py-10">
        ...
        >
          ...
          <StatCard
            ...
            value={styles.length}
            sub="garment types"
            color="bg-clay-50 text-clay-400"
          />
        </div>

        {/* Workflow Guide */}
        <div
          className="bg-ink-900 rounded-xl p-6 mb-10 animate-fade-up opacity-0-init"
          style={{ animationDelay: "200ms" }}
        >
          <p className="text-ink-400 text-xs uppercase tracking-widest mb-4">
            How it works
          </p>
            ...
            {[
              {
                ...
                desc: "Choose type, fabric, color & logo",
                to: "/design",
              },
              {
                step: "02",
                title: "Set Costs",
                desc: "Fabric, stitching, accessories",
                to: "/costing",
              },
              {
                step: "03",
                title: "Generate Sheet",
                desc: "Export buyer-ready cost sheet",
                to: "/result",
              },
              ...
              <div key={step} className="flex items-center gap-4 flex-1">
                <Link
                  to={to}
                  className="group flex items-center gap-3 flex-1 hover:opacity-80 transition-opacity"
                >
                ...
                {i < 2 && (
                  <ArrowRight
                    size={16}
                    className="text-ink-600 hidden sm:block shrink-0"
                  />
                )}
              </div>
            ))}
        ...
        {/* Designs Grid */}
        <div>
          <h2 className="font-display text-xl font-semibold text-ink-800 mb-5">
            Saved Designs
          </h2>
          ...
          {designs.length === 0 ? (
            <div className="bg-white rounded-xl border border-dashed border-ink-200 p-8 sm:p-16 text-center">
              ...
              <Link
                to="/design"
                className="text-clay-500 text-sm underline mt-1 inline-block"
              >
                Create your first →
              </Link>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
              {designs.map((design, i) => (
                <div
                  ...
                  className="bg-white rounded-xl border border-ink-100 overflow-hidden hover:shadow-md transition-shadow animate-fade-up opacity-0-init group cursor-pointer"
                ...
                >
                  ...
                  <div
                    ...
                    style={{
                      background: `linear-gradient(135deg, ${design.garmentColor}22, ${design.garmentColor}44)`,
                    }}
                  >
                    <GarmentPreview
                  ...
                  <div className="p-4">
                    <div className="flex items-start justify-between mb-1">
                      <h3 className="font-medium text-ink-900 text-sm leading-tight">
                        {design.name}
                      </h3>
                    ...
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <p className="font-mono text-sm font-semibold text-ink-900">
                          Rs. {design.cost.totalPerUnit.toLocaleString()}
                        </p>
                      </div>
                      <button
                        onClick={(e) => {
