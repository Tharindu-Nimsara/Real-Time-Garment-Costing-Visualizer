...
function Row({ label, value, bold = false, accent = false }) {
  return (
    <div
      className={`flex justify-between items-center py-1.5 ${bold ? "border-t border-ink-200 mt-1 pt-3" : ""}`}
    >
      <span
        className={`text-sm ${bold ? "font-medium text-ink-800" : "text-ink-500"}`}
      >
        {label}
      </span>
      <span
        className={`font-mono text-sm ${bold ? "font-semibold text-ink-900 text-base" : accent ? "text-clay-600" : "text-ink-700"}`}
      >
...
export default function CostSummaryCard({ compact = false }) {
  const { displayedCost, quantity } = useApp();
  const { breakdown, subtotal, totalPerUnit, totalForQuantity } = displayedCost;
    ...
    return (
      <div className="bg-ink-900 rounded-xl p-4 text-cream">
        ...
        <p className="font-mono text-2xl font-semibold">
          Rs. {totalPerUnit.toLocaleString()}
        </p>
        <p className="text-ink-400 text-xs mt-1">
          × {quantity.toLocaleString()} units ={" "}
          <span className="text-sage-300">
            Rs. {totalForQuantity.toLocaleString()}
          </span>
        </p>
      </div>
    );
  ...
  return (
    <div className="bg-white rounded-xl border border-ink-100 overflow-hidden">
      ...
      <div className="bg-ink-900 px-4 py-3 flex items-center justify-between">
        <span className="text-cream font-display text-sm font-semibold tracking-wide">
          Cost Breakdown
        </span>
      ...
      <div className="px-4 py-3 space-y-0.5">
        ...
        <div className="bg-sage-50 rounded-lg px-3 py-2.5 flex justify-between items-center">
          ...
          <span className="font-mono font-bold text-sage-700 text-base">
            Rs. {totalForQuantity.toLocaleString()}
          </span>
        </div>
      </div>
