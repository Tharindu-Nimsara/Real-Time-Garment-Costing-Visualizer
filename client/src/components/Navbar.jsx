        ...
        <div className="flex items-center gap-3">
          <div className="hidden md:flex items-center gap-1">
            {NAV_LINKS.slice(1).map(({ to }, i) => {
              ...
              const done =
                (to === "/design" &&
                  ["/costing", "/result"].includes(pathname)) ||
                (to === "/costing" && pathname === "/result");
              return (
                <div key={to} className="flex items-center gap-1">
                  <div
                    className={`w-5 h-5 rounded-full flex items-center justify-center text-[10px] font-mono transition-all ${active ? "bg-clay-400 text-white" : done ? "bg-sage-500 text-white" : "bg-ink-700 text-ink-400"}`}
                  >
                    {done ? "✓" : i + 1}
                  </div>
          ...
          {user && (
            <div className="hidden sm:flex items-center gap-2">
              ...
              <div className="hidden lg:block">
                <p className="text-cream text-xs font-medium leading-none">
                  {user.name}
                </p>
                <p className="text-ink-400 text-[10px] leading-none mt-0.5">
                  {user.role}
                </p>
              </div>
            </div>
