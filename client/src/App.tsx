...
function App() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100">
      ...
      {/* Main Content */}
      <div className="flex flex-col lg:flex-row min-h-[calc(100vh-73px)] lg:h-[calc(100vh-73px)] overflow-visible lg:overflow-hidden relative bg-white lg:bg-transparent">
        {/* Left Panel - Design Controls */}
        <aside
          className={`
            bg-white border-b lg:border-b-0 lg:border-r border-slate-200 transition-all duration-300 lg:overflow-hidden order-2 lg:order-1
            ${leftPanelOpen ? 'w-full lg:w-80' : 'hidden lg:block lg:w-0'}
            flex-shrink-0
          `}
        >
          <div className="w-full lg:w-80 h-full overflow-visible lg:overflow-y-auto pb-8 lg:pb-0">
            {/* Tabs */}
        ...
        {/* Toggle Left Panel - Hidden on Mobile */}
        <button
          onClick={() => setLeftPanelOpen(!leftPanelOpen)}
          className="hidden lg:flex absolute left-0 top-1/2 -translate-y-1/2 z-40 bg-white border border-slate-200 rounded-r-lg p-1.5 shadow-md hover:bg-slate-50 transition-all"
        ...
        {/* Center - Visualizer */}
        <main className="w-full h-[55vh] lg:h-full lg:flex-1 flex items-center justify-center p-0 lg:p-8 relative flex-shrink-0 bg-slate-50 border-b lg:border-b-0 border-slate-200 order-first lg:order-2 sticky top-[73px] lg:static z-40">
          <div className="w-full h-full lg:max-w-2xl lg:h-auto lg:aspect-[4/5] bg-slate-50 lg:rounded-2xl overflow-hidden lg:shadow-inner lg:border border-slate-200 relative">
            <ErrorBoundary>
              <Polo3DVisualizer
          ...
          <button
            ...
            className="absolute top-4 right-4 lg:top-8 lg:right-8 flex items-center gap-2 px-3 py-1.5 lg:px-4 lg:py-2 bg-white/90 backdrop-blur-sm border border-slate-200 rounded-lg shadow-md hover:bg-slate-50 transition-all z-10"
          >
          ...
          <div className="absolute bottom-4 left-1/2 -translate-x-1/2 bg-white/90 backdrop-blur-sm rounded-xl shadow-lg px-4 py-2 lg:px-6 lg:py-3 flex items-center gap-3 lg:gap-6 w-11/12 max-w-sm lg:w-auto justify-center z-10">
            <div className="flex items-center gap-2 flex-shrink-0">
              <div
                className="w-5 h-5 lg:w-6 lg:h-6 rounded-full border-2 border-white shadow"
                style={{ backgroundColor: config.color.hex }}
              />
              <span className="text-xs lg:text-sm text-slate-600 truncate max-w-[80px]">{config.color.name}</span>
            </div>
            <div className="h-5 lg:h-6 w-px bg-slate-200 flex-shrink-0" />
            <div className="text-xs lg:text-sm text-slate-600 flex-shrink-0 truncate max-w-[100px]">
              {config.fabric.name}
            </div>
            <div className="h-5 lg:h-6 w-px bg-slate-200 flex-shrink-0" />
            <div className="text-xs lg:text-sm font-semibold text-slate-900 flex-shrink-0">
              {config.quantity} u.
            </div>
          </div>
        </main>

        {/* Toggle Right Panel - Hidden on Mobile */}
        <button
          onClick={() => setRightPanelOpen(!rightPanelOpen)}
          className="hidden lg:flex absolute right-0 top-1/2 -translate-y-1/2 z-40 bg-white border border-slate-200 rounded-l-lg p-1.5 shadow-md hover:bg-slate-50 transition-all"
        ...
        <aside
          className={`
            bg-white border-t lg:border-t-0 lg:border-l border-slate-200 transition-all duration-300 lg:overflow-hidden order-3
            ${rightPanelOpen ? 'w-full lg:w-96' : 'hidden lg:block lg:w-0'}
            flex-shrink-0
          `}
        >
          <div className="w-full lg:w-96 h-full lg:overflow-y-auto p-4 space-y-6 pb-24 lg:pb-4">
            {/* Costing Panel */}
            <CostingPanel
